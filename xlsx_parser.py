...
def _parse_plancraft_format(ws: Worksheet) -> List[Position]:
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):  # Header überspringen
        ...
        # Menge
        try:
            menge = float(str(menge_raw).replace(".", "").replace(",", ".")) if menge_raw is not None else None
        ...
        # Einheitspreis
        try:
            ep = float(str(einheitspreis).replace(".", "").replace(",", ".")) if einheitspreis is not None else None
        except (ValueError, TypeError):
            ep = None
...
def _parse_foerderantrag_format(ws: Worksheet) -> List[Position]:
    ...
    for i, row in enumerate(ws.iter_rows(min_row=header_row_idx + 1, values_only=True), header_row_idx + 1):
        ...
        try:
            ep = float(str(preis_raw).replace(".", "").replace(",", ".")) if preis_raw is not None else None
        except (ValueError, TypeError):
            ep = None
...
def erkenne_format(ws: Worksheet) -> str:
    ...
    """
    # Erste Zeile lesen
    erste_zeile = next(ws.iter_rows(min_row=1, max_row=1, values_only=True), None)
    if not erste_zeile:
        return "unbekannt"

    # Plancraft-Format: 5 Spalten, "Menge" in Spalte A
    if len(erste_zeile) == 5 and erste_zeile[0] and "menge" in str(erste_zeile[0]).lower():
        return "plancraft"

    # Förderantrag-Format: 9+ Spalten, "Pos." oder "Beschreibung" in Header
    if len(erste_zeile) >= 5 and erste_zeile[0] and "pos" in str(erste_zeile[0]).lower():
        return "foerderantrag"

    # Fallback: Wenn "Beschreibung" irgendwo in den ersten 3 Zeilen auftaucht
